...
router.get("/history", authMiddleware, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const formattedMatches = matches.map((m: (typeof matches)[number]) => {
...
// GET /api/v1/match/summary/:sessionId (Protected)
router.get("/summary/:sessionId", authMiddleware, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { sessionId } = req.params;
    const aiResponse = await axios.get(`${PYTHON_AI_URL}/performance/summary/${sessionId}`);
    ...
    res.status(502).json({
      success: false,
      error: "Failed to fetch match summary from AI Engine",
    });
  }
});