...
// Helpers to map Yash's frontend 0-3 indices to Aditya's A-D strings
const indexToLetter = (idx: number) => ["A", "B", "C", "D"][idx];
const letterToIndex = (letter: string) => ["A", "B", "C", "D"].indexOf(letter);
...
export class Game {
  ...
  private start() {
    this.player1.send({
      ...
      payload: {
        gameId: this.gameId,
        opponent: { id: this.player2.id, username: this.player2.username, name: this.player2.name, points: this.player2.points, avatar: this.player2.avatar },
        totalQuestions: this.questions.length,
        timeLimit: QUESTION_TIME_LIMIT_SEC,
        scoring: { correct: `+${NEET_SCORING.CORRECT}`, wrong: `${NEET_SCORING.WRONG}`, unanswered: `${NEET_SCORING.UNANSWERED}` },
      },
    });
    ...
    this.player2.send({
      ...
      payload: {
        gameId: this.gameId,
        opponent: { id: this.player1.id, username: this.player1.username, name: this.player1.name, points: this.player1.points, avatar: this.player1.avatar },
        totalQuestions: this.questions.length,
        timeLimit: QUESTION_TIME_LIMIT_SEC,
        scoring: { correct: `+${NEET_SCORING.CORRECT}`, wrong: `${NEET_SCORING.WRONG}`, unanswered: `${NEET_SCORING.UNANSWERED}` },
      },
    ...
    setTimeout(() => { this.sendQuestion(); }, 1500);
  }
  ...
  private sendQuestion() {
    ...
    this.timer = setTimeout(() => { this.evaluateQuestion(); }, QUESTION_TIME_LIMIT_SEC * 1000);
  }

  // INTEGRATION: Calling Python AI for evaluation
  public async submitAnswer(user: User, questionId: string, selectedOption: number) {
    if (this.isFinished) return;
    ...
    try {
      const aiResponse = await axios.post(`${PYTHON_AI_URL}/answers/submit`, {
        session_id: this.gameId,
        user_id: user.id,
        match_id: this.gameId,
        question_id: Number(questionId),
        chosen_answer: indexToLetter(selectedOption)
      });

      const isCorrect = aiResponse.data.is_correct;
      // Store AI's correct answer so evaluateQuestion can use it
      currentQ.correctAnswer = letterToIndex(aiResponse.data.correct_answer);
      currentQ.explanation = `Source: ${aiResponse.data.source_url}`;
      ...
      if (isP1) {
        ...
        this.player1.send({
          ...
          payload: { questionId, selectedOption, isCorrect, scoreDelta, totalScore: this.player1Score },
        });
      } else {
        ...
        this.player2.send({
          type: ANSWER_ACCEPTED,
          payload: { questionId, selectedOption, isCorrect, scoreDelta, totalScore: this.player2Score },
        });
      }
  ...
  private evaluateQuestion() {
    ...
    const resultPayload = {
      ...
      payload: {
        ...
        player1: { id: this.player1.id, name: this.player1.name, selectedOption: p1Ans, isCorrect: p1Correct, scoreDelta: p1Delta, totalScore: this.player1Score },
        player2: { id: this.player2.id, name: this.player2.name, selectedOption: p2Ans, isCorrect: p2Correct, scoreDelta: p2Delta, totalScore: this.player2Score },
      },
    };

    const scoreUpdatePayload = {
      type: SCORE_UPDATE,
      payload: { gameId: this.gameId, scores: { [this.player1.id]: this.player1Score, [this.player2.id]: this.player2Score } },
    };
  ...
  private async endGame() {
    ...
    // INTEGRATION: Notify Python AI that session is over
    try {
      await axios.post(`${PYTHON_AI_URL}/performance/end/${this.gameId}`);
    } catch (e) {
      console.error("Failed to close AI session:", e);
    }

    let winnerId: string | null = null;
    if (this.player1Score > this.player2Score) winnerId = this.player1.id;
    else if (this.player2Score > this.player1Score) winnerId = this.player2.id;
    ...
    const gameOverPayload = {
      ...
      payload: {
        ...
        finalScores: { [this.player1.id]: this.player1Score, [this.player2.id]: this.player2Score },
      },
    };
  ...
  public async handleDisconnect(user: User) {
    ...
    remainingPlayer.send({
      ...
      payload: { gameId: this.gameId, winnerId, message: "Opponent disconnected. You win by default!" },
    });
